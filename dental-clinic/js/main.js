/* =========================================================
   ...
   7) accessible form validation (name + tel required, email optional)
   8) mobile sticky CTA (hide over #reserve / footer)
...
(function () {
  ...
  function updateParallax() {
    ...
    var offset = Math.min(window.scrollY * 0.16, 48);
  ...
  /* ---------------------------------------------------------
     8) FORM validation + demo submit (no real send)
        Required: name + tel. Email is optional but format-checked.
     --------------------------------------------------------- */
  ...
  if (form) {
    ...
    // Loose client-side sanity checks only — NOT authoritative. Server-side
    // validation must confirm validity.
    var emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    // Accept digits, spaces, hyphens, parens and a leading +; require >= 10 digits.
    var telRe = /^(?=(?:[^0-9]*[0-9]){10})[0-9+\-\s()]+$/;
    ...
    function validateTel() {
      ...
      if (!v) {
        setError(telInput, 'お電話番号をご入力ください。');
        return false;
      }
      if (!telRe.test(v)) {
        setError(telInput, '電話番号の形式をご確認ください（数字10桁以上）。');
        return false;
      }
      setError(telInput, '');
      return true;
  ...
  /* ---------------------------------------------------------
     9) MOBILE STICKY CTA: hide while the #reserve section OR the
        footer is visible, to avoid overlapping the form / links.
     --------------------------------------------------------- */
  var stickyCta = document.querySelector('.sticky-cta');
  var reserveSection = document.getElementById('reserve');
  var footerEl = document.querySelector('.footer');
  if (stickyCta && reserveSection && 'IntersectionObserver' in window) {
    var hideZones = [reserveSection];
